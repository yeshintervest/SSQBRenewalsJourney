"""
Quote & Buy journey screenshotter (public, no login).

Walks the live Quote & Buy journey, taking two screenshots of each page
(empty on arrival, and filled in before Continue). On every page it also
reads Optimizely's on-page data layer and flags any page the visitor was
bucketed into an A/B test for.

Saves full-page screenshots into screenshots/quote/<date>/, writes a per-run
index.html, and rebuilds screenshots/quote/catalogue.html linking every run.
"""
...
OUTPUT_ROOT = Path("screenshots") / "quote"   # quote journey lives in its own folder
