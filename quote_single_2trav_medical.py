"""
Quote & Buy journey screenshotter -- SINGLE TRIP, 2 travellers, with medical.
...
folder differ. This variant adds a second traveller and a full medical
declaration (asthma + screening), so it is the longest of the four.
"""
...
def select_travel_dates(page):
    """Pick a future departure + return on the single-trip date page.

    This page has a 'from' box and an 'until' box; clicking each opens the
    calendar, where you click a day-number cell (as in your recording). We pick
    dates a couple of weeks out so they're always valid, and step the calendar
    forward a month if those days have already passed in the month on screen.
    """
    ...
    _click_calendar_day(page, depart.day)
    page.get_by_role("textbox", name="until").click()
    _click_calendar_day(page, ret.day)
...
# --------------------------------------------------------------------------
# THE QUOTE & BUY JOURNEY -- SINGLE TRIP, 2 travellers, with medical
...
def capture_quote_journey(shotter):
    ...
    # Page 2 - Travelling to (single trip picks a named destination)
    ...
    # Page 4 - Travel dates (departure + return)
    ...
    # regenerated on every page load so it can't be reused. Robust selector:
    page.get_by_role("option", name="Ms", exact=True).click()
    page.get_by_test_id("first-name-organiser-input").click()
    page.get_by_test_id("first-name-organiser-input").fill("test one")
    ...
    page.get_by_role("listitem").filter(has_text="Midsummer House, Forden").click()
    ...
    page.get_by_test_id("fist-name-wrapper-0").click()
    page.get_by_test_id("traveller-first-name-1-input").fill("test two")
    page.get_by_test_id("lastname-wrapper-0").click()
    page.get_by_test_id("traveller-last-name-1-1-input").fill("test")
    page.get_by_test_id("traveller-form-year-0").click()
    page.get_by_test_id("traveller-form-day-0").fill("01")
    page.get_by_test_id("traveller-form-month-0").fill("02")
    ...
    # Page 8 - Medical confirmation (kept exactly as recorded)
    ...
    # rather than the exact value from the recording.
    ...
    page.get_by_label("AsthmaCardiac").get_by_text("Asthma", exact=True).click()
    page.get_by_role("button", name="Start screening").click()

    # --- Asthma screening questionnaire -------------------------------------
    # The questions appear one at a time, so we answer the one currently on
    # screen each time. Matching is exact (^No$ / ^0$) so "No" doesn't also grab
    # "None"/"Nothing" etc., and .first targets the visible question. There's an
    # extra guarded "No" right before Finish -- that last question was the one
    # being missed on the previous run.
    def click_no():
    ...
    click_no()
    click_no()
    # the "No" that was being missed just before finishing (harmless if the
    # questionnaire is already complete -- it just finds nothing and moves on)
    try:
        click_no()
    except Exception:
        pass
    ...
    # a second Finish/confirm sometimes follows; click it only if it's there
    ...
    page.get_by_test_id("medical-dashboard-submit-button").click()
    ...
    page.get_by_test_id("contact-mobile-input").fill("03335555333")
    ...
    # Page 13 - Quote review (nothing to fill here; recording stops at the quote)
