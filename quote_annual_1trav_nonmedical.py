"""
Quote & Buy journey screenshotter -- ANNUAL multi-trip, 1 traveller, no medical.
...
This is the original working quote_journey.py journey, unchanged -- only the
output folder (now per-variant) and the catalogue function differ.
"""
...
VARIANT = "annual-1trav-nonmedical"

# Travel start date is generated ~30 days ahead every run, so it never
# becomes a past (invalid) date. Annual multi-trip only needs a start date.
...
# --------------------------------------------------------------------------
# THE PUBLIC QUOTE & BUY JOURNEY  (no login) -- ANNUAL, 1 traveller, no medical
# Steps recorded from the live site; a screenshot is taken on arrival at each
# page (before filling it in), so you see the page as a customer does.
# --------------------------------------------------------------------------
